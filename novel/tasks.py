...
def get_baidu_top_books():
    ...
    for url in (XHQH, DSYQ, CYJK, QCXS, WXXX):
        try:
            ...
            soup = BeautifulSoup(_get.content.decode(
                'gbk', 'ignore'), "html.parser")
            # print(soup)
            for tr in soup.find_all('tr'):
                # print(tr);break;
                try:
                    bookname = tr.find(name='td', attrs={
                                       'class': 'keyword'}).a.text.strip()
                    bookhotcount = tr.find(
                        name='td', attrs={'class': 'last'}).text.strip()
...
def record_hot_book(bookname, hot_number):
    try:
        _ = requests.get(
            'http://novels.freelycode.com/search/?bookname={0}'.format(bookname))
    except:
        pass
    if Book.objects.filter(name=bookname).exists():
        try:
            book = Book.objects.filter(
                name=bookname).update(hot=int(hot_number))
        except:
            logging.error('更新热度失败bookname:{0} hotnumber:{1}'.format(
                bookname, hot_number))
    return True
