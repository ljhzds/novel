...
def chapter(request, book_id, index):
    ...
    try:
        ...
        chapter = BookChapter.objects.filter(
            book=book).filter(index=index).first()
        ...
        for index, chapter in enumerate(chapters):
            ...
            if BookChapter.objects.filter(book=book).filter(index=index).exists():
                ...
                bookchapter = BookChapter(
                    book=book, title=chapter_title, url=url, index=index)
                bookchapter.save()
    except Book.DoesNotExist:
...
def download(request, book_id):
    ...
    book = Book.objects.get(pk=book_id)
    book.download_times += 1
    book.save()
    file_name = book.name + '.txt'

    def downloadbook():
        chapters = BookChapter.objects.filter(
            book_id=book_id).order_by('index')
        for chapter in chapters:
            title = chapter.title
            content = get_chapter_content(
                chapter_url=chapter.url, book_website=book.source_site)
            yield title + '\n' + content + '\n'

    response = StreamingHttpResponse(downloadbook())
    response['Content-Type'] = 'application/octet-stream'
    response[
        'Content-Disposition'] = 'attachment;filename="{0}"'.format(urlquote(file_name))
